---
description: "Web profile bundle layer，为 Markdown、JSON、CSV/TSV、图片、Office 文件、patch、SVG 与视频添加右侧栏渲染式产物 viewer。"
kind: "package-bundle"
---
...
`dsh-artifact-viewers` 为自定义 Web profile 添加右侧栏渲染式文件 preview。Markdown、JSON、CSV/TSV、常见图片文件、PDF、DOCX、XLSX、unified diff 文件、经过净化的 SVG 与视频会用理解格式的 viewer 打开，未知扩展名继续使用 text fallback。默认 `dsh-web-app` bundle 已经包含相同的 viewer row，因此只有在组合自定义 Web 表层时才安装这一层。该层插入五条 viewer 插件 row，不添加模型可见行为。
...
这些 row 贡献用于 Markdown、JSON、CSV/TSV、常见浏览器图片格式、PDF、DOCX、XLSX、unified diff 文件、经过净化 SVG 与视频的右侧栏 preview tab type。格式行为与限制由对应的 client package README 负责。
...
该 bundle 是一个静态 patch document，包含一个 `insert` list。每条 row id 都与默认 Web bundle 使用的 id 相同，因此同时应用两层的 profile 会定位同一批 row，而不是挂载重复 viewer。Loader row 顺序不定义激活顺序；插入的插件依赖 Web profile 提供的右侧栏、slot registry、locale service、Remote 服务与 `file` resource provider。
...
- [Video preview client plugin](../../client/ui-sidebar-video-preview/README.zh.md) - 原生播放与 Range 交付。
...
无，因为 bundle 插入的是 viewer row，不注册 tool、prompt section 或 session event。
...
- **Viewer 限制属于插件** - 文件格式、图片大小限制、SVG 净化、patch 语法、视频 Range 交付与 first-page text 行为由被插入的 client plugin 文档说明。
