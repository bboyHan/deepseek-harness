/** Address translation and Remote bindings for office preview reads. */
...
/** Client Remote subset used by the office viewer. */
export interface WorkspaceFilesOfficeRemote {
...
/** Read one byte window for an office tab. */
export type ReadOfficeBytes = (
  sessionId: SessionId,
  path: string,
  offset: number,
  signal: AbortSignal,
...
export function hostFileOf(address: string, sessionId: SessionId): SessionFile {
  ...
  if (parsed === undefined) {
    throw new Error(`ui-sidebar-office-preview: not a file address "${address}"`)
  }
 ...
 * @param remote - Client Remote carrying workspaceFiles.
 * @returns the byte-window reader.
 */
export function createReadBytes(remote: WorkspaceFilesOfficeRemote): ReadOfficeBytes {
